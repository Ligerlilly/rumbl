defmodule Rumbl.VideoControllerTest do
  ...
  @tag login_as: "Max"
  test "authorizes actions against access be other users", %{user: owner, conn: conn} do
  ...
  @tag login_as: "Max"
  test "creates user vieeo and redirects", %{conn: conn, user: user} do
  ...
  test "requires user authentication on all actions", %{conn: conn} do
    Enum.each([
      ...
      get(conn, video_path(conn, :update, "1", %{})),
      get(conn, video_path(conn, :create, %{})),
      get(conn, video_path(conn, :delete, "1")),
      ], fn conn ->
